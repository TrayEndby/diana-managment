...
const SavedFiltersModal = ({ location, history, onHide, storageKey }) => {
  ...
  const addToSaved = () => {
    ...
    storageService.set(storageKey, JSON.stringify(newSavedSearches));
  ...
  const finishChanging = () => {
    storageService.set(storageKey, JSON.stringify(savedSearches));
    setIsEditMode(false);
  };
